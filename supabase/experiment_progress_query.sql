-- Every Web Experiment session — who picked A/B and how far they got,
-- regardless of whether the session ever linked to a real account. Driven
-- from experiment_sessions (not auth.users/profiles), which is the fix for
-- the "I saw my results but I'm not in the SQL" problem: linking only
-- happens once, right after signup (completeExperimentTask1() in
-- lib/experiment.js) — if that write ever fails/races, the session keeps
-- recording real task timestamps off its cookie with no account attached,
-- so a query that starts FROM accounts can never find it. Starting from
-- sessions instead means nothing is invisible.
--
