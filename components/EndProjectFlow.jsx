  ...
  if (step === "rate") {
    return (
      <div className="mt-6 flex flex-col gap-4 px-6">
        ...
        {members.map((m) => <RateForm key={m.userId} rateeId={m.userId} name={m.name} />)}
  ...
  if (step === "addProject") {
    ...
    return (
      <div className="mt-6 flex flex-col gap-3 px-6 text-center">
        ...
        <button onClick={onAddProject} disabled={saving || !role.trim()} className="mt-2 w-full rounded-2xl bg-gradient-to-r from-purple-600 to-purple-700 py-3.5 text-[15px] font-bold text-white disabled:opacity-50">
