...
export default function ApplicantReviewBar({ requestId, groupId, applicantId, prevHref, nextHref }) {
  ...
  async function onAccept() {
    ...
    setBusy(false);
    if (!r?.error) { setDone("accepted"); router.refresh(); }
  }
  async function onDecline() {
    setBusy(true);
    const r = await declineRequest(requestId);
    setBusy(false);
    if (!r?.error) { setDone("declined"); router.refresh(); }
  }

  return (
    <div className="fixed bottom-0 left-1/2 z-20 flex w-[402px] -translate-x-1/2 items-center justify-between gap-3 border-t border-line bg-white px-6 py-3">
      {prevHref ? (
