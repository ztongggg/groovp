...
// EXPERIMENT: includePersonality=false for the Neutral (B) arm's viewer —
// see lib/experiment.js — drops the personality dimension from scoring
// entirely rather than just not displaying it.
export function computeMatch(applicant, group, { includePersonality = true } = {}) {
  ...
  const matchedPersonality = includePersonality ? overlap(group?.personality_wanted, applicantPersonalityTerms(applicant)) : [];
