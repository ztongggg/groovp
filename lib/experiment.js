...
// (intro, results, admin), components/SignupForm.jsx (revert
// app/signup/page.jsx to hold that content directly again),
// supabase/schema_v14.sql, the <ExperimentTimerOverlay/> line in
// app/layout.jsx, the "Start Web Experiment A/B" block in
// app/login/page.jsx, the "/experiment/intro" entry in
// lib/supabase/middleware.js's PUBLIC_PATHS, and the handful of one-line
// calls into this module (or getExperimentCondition/isNeutral props) in
// app/signup, app/tutorial/actions.js, app/discover/actions.js,
// app/create, app/applicants, app/groups/[groupId], app/u/[id],
// app/profile, app/recruiting/[groupId], components/ProfileParts.jsx,
// components/ProfileView.jsx, components/UserProfileView.jsx,
// components/RecruitingForm.jsx, and lib/matching.js's
// `includePersonality` option — every touch point is tagged "EXPERIMENT:"
// so they're a grep away. (components/NotificationToast.jsx and
...
// The landing screen shows two plain "A"/"B" buttons — participants never see
// which arm they're in. "A" = today's real matching (Personalised), "B" =
// Neutral, work-style/personality stripped everywhere a participant could
// see it. Whoever sends the link picks A or B; this isn't randomized.
const CONDITION_BY_ARM = { A: "personalised", B: "neutral" };
...
// Clicked ("Begin") from the consent screen. Creates the session row, starts
// Task 1's clock immediately (landing on Signup IS the first page of Task
// 1), and hands off into the real signup flow.
export async function startExperimentSession(arm) {
  const condition = CONDITION_BY_ARM[arm] || CONDITION_BY_ARM.A;
...
// Read server-side by any page/component that needs to know which arm the
// current viewer is in — e.g. to hide work-style/personality UI for Neutral.
// Returns "personalised" | "neutral" | null (no active session — the
// overwhelming majority of page loads, real users included).
export async function getExperimentCondition() {
  const sid = getSid();
  if (!sid) return null;
  const supabase = createClient();
  const { data } = await supabase.from("experiment_sessions").select("condition").eq("id", sid).maybeSingle();
...
export async function getExperimentStatus() {
  ...
  // Self-healing link: completeExperimentTask1() is the only place user_id
  // normally gets set, right after signup — if that single write ever
  // failed/raced, the session keeps recording real task timestamps with no
  // account attached, invisible to any query that starts from accounts.
  // Every poll while this session is active already has both pieces on
  // hand, so repair the link opportunistically instead of leaving it
  // permanently orphaned.
  if (user && !session.user_id) {
    await supabase.from("experiment_sessions").update({ user_id: user.id }).eq("id", sid).is("user_id", null);
  }
