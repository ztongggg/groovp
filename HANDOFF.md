...
- LinkedIn verify: Settings "Verify with LinkedIn" (`linkIdentity` linkedin_oidc) → `/auth/callback` sets `linkedin_verified` → blue badge on `/profile` + `/u/[id]`. **Needs provider setup — see `supabase/LINKEDIN_SETUP.md`.**
